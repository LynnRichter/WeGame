...
-(void)defaultSettings
{
    ...
    if (self.hasBoard) {
        btn.layer.borderColor = [RGBClor(238, 238, 238) CGColor];
    ...
    titleLabel.textColor = RGBClor(74, 74, 74);
    [btn addSubview:titleLabel];
    
    _arrow = [[UIImageView alloc]initWithFrame:CGRectMake(btn.frame.size.width - imgW - 4, (self.frame.size.height-imgH)/2.0, imgW, imgH)];
    ...
    if (self.hasBoard) {
        ...
        _listTable.layer.borderColor = [RGBClor(238, 238, 238) CGColor];;
    }
   
    
    [_supView addSubview:_listTable];
    
    titleLabel.text =  [NSString stringWithFormat:@"%@",[[_titlesList objectAtIndex:_defaultIndex] objectForKey:self.key]] ;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell==nil)
    {
        ...
        label.textColor = RGBClor(74, 74, 74);
        label.tag = 1000;
        [cell addSubview:label];
        
        UIImageView *line = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.frame.size.height-0.5, self.frame.size.width, 0.5)];
        line.backgroundColor = RGBClor(238, 238, 238) ;
        [cell addSubview:line];
    }
    UILabel *label = (UILabel *)[cell viewWithTag:1000];
    label.text = [[_titlesList objectAtIndex:indexPath.row] objectForKey:self.key];
...
{
    titleLabel.text = [[_titlesList objectAtIndex:indexPath.row] objectForKey:self.key];
    _isOpen = YES;
    [self tapAction];
